...
def main(args = None) -> int:
    try:
        ...
        parser.add_argument("--metadata", help="The metadata yaml file", type=Path, required=False, default="metadata/metadata.yml")
        options = parser.parse_args(args)


        log.info("Testing if %s needs updates using labels in %s", options.compose, options.metadata)
        # get available jsons
        ...
        if update_compose_labels(compose_cfg, json_metadata):
            log.info("Updating %s using labels in %s", options.compose, options.metadata)
            # write the file back
